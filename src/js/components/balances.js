...
function SweepModalViewModel() {
  ...
  self.addressForPrivateKey.subscribe(function(address) {
    ...
    self.sweepAssetsCost = {'SFR': MIN_FEE+REGULAR_DUST_SIZE};
  ...
  self.addressForPrivateKeyForFees.subscribe(function(address) {
    ...
    WALLET.retriveBTCAddrsInfo([address], function(data) {
      $.jqlog.debug(data);
      self.addressForFeesBalanceMessage(normalizeQuantity(data[0]['confirmedRawBal'])+' SFR in '+address);
      ...
      WALLET.BITCOIN_WALLET.getOldAddressesInfos(function(data) {
        for (var address in data) {
          if (self.excludedOldAddresses.indexOf(address) == -1) {
            self.availableOldAddresses.push(new BalancesAddressInDropdownItemModel(address, address, data[address]['SFR']['privkey']));
          }       
        }
      }); 
    }    
  ...
  self._doSendAsset = function(asset, key, pubkey, opsComplete, adjustedBTCQuantity, callback) {
    ...
    //TODO: remove this
    if(asset == 'SFR') assert(adjustedBTCQuantity !== null);
    ...
    var quantity = (asset == 'SFR') ? (self.btcBalanceForPrivateKey() - MIN_FEE) : selectedAsset.RAW_BALANCE;
    var normalizedQuantity = (asset == 'SFR') ? normalizeQuantity(quantity) : selectedAsset.NORMALIZED_BALANCE;
    
    assert(selectedAsset);
    
    if(!quantity) { //if there is no quantity to send for the asset, only do the transfer
      if(asset == 'cSFR' || asset == 'SFR') { //nothing to send, and no transfer to do
  ...
  self.doAction = function() {
    ...
    if(hasBTC !== false) {
      ...
      sendsToMake.push(["SFR", cwk, pubkey, opsComplete, self.btcBalanceForPrivateKey()]);
    }
    ...
    var launchSweep = function() {
      if (sendsToMake.length==1 && sendsToMake[0][0]=='SFR') {
        doSweep();
      } else {
...
function TestnetBurnModalViewModel() {
  ...
  self.btcBurnQuantity = ko.observable('').extend({
    ...
    validation: [{
      validator: function (val, self) {
        return parseFloat(val) > 0 && parseFloat(val) <= 30000;
      },
      ...
      validator: function (val, self) {
        return !(parseFloat(val) > 30000 - self.btcAlreadyBurned());
      },
  ...
  self.maxPossibleBurn = ko.computed(function() { //normalized
    ...
    return Math.min(30000 - self.btcAlreadyBurned(), WALLET.getAddressObj(self.address()).getAssetObj('SFR').normalizedBalance()) 
  }, self);
