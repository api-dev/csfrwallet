  ...
  self.completeBTCPay = function() {
    ...
    WALLET.retrieveBTCBalance(self.BTCPAY_DATA['myAddr'], function(balance) {
      if(balance < self.BTCPAY_DATA['btcQuantityRaw'] + MIN_PRIME_BALANCE) {
        bootbox.alert("You do not have the required <b class='notoAssetColor'>SFR</b> balance to settle this order."
          + " Please deposit more <b class='notoAssetColor'>SFR</b> into address"
      ...
      bootbox.dialog({
        message: "Confirm a payment of <b class='notoQuantityColor'>" + self.BTCPAY_DATA['btcQuantity'] + "</b>"
          + " <b class='notoAssetColor'>SFR</b>" + " to address"
        ...
        title: "Confirm Order Settlement (SFR Payment)",
        buttons: {
          cancel: {
            label: "Cancel",
    ...
    failoverAPI("get_order_matches", {'filters': filters, 'filterop': 'or', status: 'pending'},
      function(data, endpoint) {
        ...
        for(var i=0; i < data.length; i++) {
          ...
          if(   WALLET.getAddressObj(data['tx0_address']) && data['forward_asset'] == 'SFR'
             || WALLET.getAddressObj(data['tx1_address']) && data['backward_asset'] == 'SFR')
             continue;
...
WaitingBTCPayFeedViewModel.makeBTCPayData = function(data) {
  ...
  var firstInPair = (WALLET.getAddressObj(data['tx0_address']) && data['forward_asset'] == 'SFR') ? true : false;
  if(!firstInPair) if (!(WALLET.getAddressObj(data['tx1_address']) && data['backward_asset'] == 'SFR')) return false;
...
function UpcomingBTCPayFeedViewModel() {
  ...
  self.process = function(btcPayData) {
    ...
    if (PROCESSED_BTCPAY[btcPayData['orderMatchID']]) {
      $.jqlog.error("Attempt to make duplicate btcpay: " + btcPayData['orderMatchID']);
      return false;
    } else if (btcPayData['matchExpireIndex'] - WALLET.networkBlockHeight() <= 6) {
      $.jqlog.error("Attempt to make expired btcpay: " + btcPayData['orderMatchID']);
      return false;
    } else {
    ...
    if(PREFERENCES['auto_btcpay']) {
      ...
      if(WALLET.getBalance(btcPayData['myAddr'], 'SFR', false) >= (btcPayData['btcQuantityRaw']) + MIN_PRIME_BALANCE) {
        ...
        WALLET.doTransaction(btcPayData['myAddr'], "create_btcpay",
          ...
          function(txHash, data, endpoint, addressType, armoryUTx) {
            ...
            var message = "Automatic <b class='notoAssetColor'>SFR</b> payment of "
              + "<b class='notoQuantityColor'>" + btcPayData['btcQuantity'] + "</b>"
              + " <b class='notoAssetColor'>SFR</b> made from address"
            ...
            WAITING_BTCPAY_FEED.add(btcPayData);
            bootbox.alert("There was an error processing an automatic <b class='notoAssetColor'>SFR</b> payment."
        ...
        WALLET.showTransactionCompleteDialog("A payment on a matched order for "
          + "<b class='notoQuantityColor'>" + btcPayData['btcQuantity'] + "</b>"
          + "<b class='notoAssetColor'>SFR</b> is required, however, the address that made the order ("
          ...
          + "<br/><br/>Please deposit the necessary <b class='notoAssetColor'>SFR</b> into this address and"
          + "manually make the payment from the Saffroncoin icon in the top bar of the site.");
      }
      ...
      var prompt = "An order match for <b class='notoQuantityColor'>" + btcPayData['otherOrderQuantity'] + "</b>"
        ...
        + " <b class='notoAssetColor'>SFR</b>" + " from address"
      ...
      bootbox.dialog({
        message: prompt,
        title: "Order Settlement (SFR Pay)",
        buttons: {
          success: {
            label: "No, hold off",
            ...
            callback: function() {
              WALLET.doTransaction(btcPayData['myAddr'], "create_btcpay",
                ...
                function(txHash, data, endpoint, addressType, armoryUTx) {
                  //notify the user of the automatic BTC payment
                  var message = "Automatic <b class='notoAssetColor'>SFR</b> payment of"
                    + " <b class='notoQuantityColor'>" + btcPayData['btcQuantity'] + "</b> <b class='notoAssetColor'>SFR</b> "
                  ...
                  WAITING_BTCPAY_FEED.add(btcPayData);
                  bootbox.alert("There was an error processing an automatic <b class='notoAssetColor'>SFR</b> payment."
                    + "<br/><br/><b>Please manually make the payment from the Saffroncoin icon in the top bar of the site.</b>");
                }
              );
            }
          },
