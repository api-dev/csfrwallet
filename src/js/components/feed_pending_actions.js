...
PendingActionViewModel.calcText = function(category, data) {
  ...
  if(category == 'burns') {
    desc = pending + " burn of <Am>" + normalizeQuantity(data['quantity']) + "</Am> <As>SFR</As>";
    ...
    desc = pending + " <b>" + data['bet_type'] + "</b> bet on feed @ <Ad>"
      ...
      + "Wager: <Am>"
      + numberWithCommas(normalizeQuantity(data['wager_quantity'])) + "</Am> <As>cSFR</As>, Counterwager: <Am>"
      + numberWithCommas(normalizeQuantity(data['counterwager_quantity'])) + "</Am> <As>cSFR</As>";
    ...
    desc = pending + " SFR Payment from <Ad>" + getAddressLabel(data['source']) + "</Ad>";
  } else if(category == 'rps') {
    desc  = pending + "  RPS game with <Ad>" + getAddressLabel(data['source']) + "</Ad>: ";
    desc += " <Am>"+numberWithCommas(normalizeQuantity(data['wager'])) + '</Am> <As>cSFR</As>';
    ...
    if (WALLET.getAddressObj(data['tx1_address']) && data['forward_asset'] == 'SFR' && data['_status'] == 'pending') {
      desc = "Waiting <Am>" + numberWithCommas(normalizeQuantity(data['forward_quantity'])) + "</Am> <As>SFR</As> payment from <Ad>" + getAddressLabel(data['tx0_address']) + "</Ad>";
    } else if (WALLET.getAddressObj(data['tx0_address']) && data['backward_asset'] == 'SFR' && data['_status'] == 'pending') {
      desc = "Waiting <Am>" + numberWithCommas(normalizeQuantity(data['backward_quantity'])) + "</Am> <As>SFR</As> payment from <Ad>" + getAddressLabel(data['tx1_address']) + "</Ad>";
    }
...
function PendingActionFeedViewModel() {
  ...
  self.pendingSellBTCOrdersCount = ko.computed(function() {
    return $.map(self.entries(), function(item) { 
        var sellingBTC = ('orders' == item.CATEGORY && 'SFR' == item.DATA.give_asset) || ('btcpays' == item.CATEGORY);
  ...
  self.remove = function(txHash, category, btcRefreshSpecialLogic) {
    ...
    if(match) {
      ...
      // send txns (e.g. is a csfr asset send, or asset issuance, or something the BTC balance refresh
      ...
      if(btcRefreshSpecialLogic) {
        ...
        if (match['CATEGORY'] != category || match['DATA']['asset'] != 'SFR')
          return;
...
PendingActionFeedViewModel.modifyBalancePendingFlag = function(category, data, flagSetting) {
  ...
  if(category == 'burns') {
    ...
    addressObj.getAssetObj("cSFR").balanceChangePending(flagSetting);
    updateUnconfirmedBalance(data['source'], "SFR", data['quantity'] * -1);
    ...
    updateUnconfirmedBalance(data['source'], "SFR", data['quantity'] * -1);
    updateUnconfirmedBalance(data['destination'], "SFR", data['quantity']);
    ...
    if(assetObj && assetObj.isMine()) {
      ...
      // issuance fee
      updateUnconfirmedBalance(data['source'], 'cSFR', -ASSET_CREATION_FEE_XCP * UNIT);
    }
    ...
    if (data['give_asset'] != 'SFR') {
      updateUnconfirmedBalance(data['source'], data['give_asset'], data['give_quantity'] * -1);
    }   

  } else if (category == 'bets') {

    updateUnconfirmedBalance(data['source'], 'cSFR', data['wager_quantity'] * -1);
    
  } else if (category == 'rps') {

    updateUnconfirmedBalance(data['source'], 'cSFR', data['wager'] * -1);
