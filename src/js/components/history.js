function BalanceHistoryViewModel() {
  ...
  //^ don't load SFR as an asset on testnet, since we can't show the data (since blockchain doesn't support testnet)
  self.graphData = null;
  ...
  self.init = function() {
    ...
    failoverAPI("get_owned_assets", { addresses: WALLET.getAddressesList() }, function(data, endpoint) {
      ...
      //^ this way, cSFR and SFR stay at the top, but the other assets are alphabetically sorted
    });
  ...
  self.assetChanged = function() {
    ...
    if(self.selectedAsset() == "SFR") { //mainnet only (as we use blockchain.info for this and they don't support testnet)
      ...
      for(var i=0; i < addresses.length; i++) {
        //since we don't track SFR balances, we need to go to blockchain.info for that
